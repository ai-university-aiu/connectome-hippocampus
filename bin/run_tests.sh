#!/usr/bin/env bash
# run_tests.sh — run every region pack's in-pack PLUnit suite.
#
# Each pack ships packs/<name>/test/test_<name>.pl. The region's suite TRIES to make
# the region confabulate and confirms it cannot (the membership contract refuses a
# non-member). Exit 0 iff all pass.
set -u
...
# Build the library path over every region pack plus the reused PrologAI dirs.
LIB=""
for d in packs/*/prolog; do LIB="$LIB -p library=$d"; done
# The strata runtime and neural_lattice need PrologAI's lattice and actors packs.
LIB="$LIB -p library=$PROLOGAI_HOME/packs/lattice/prolog"
LIB="$LIB -p library=$PROLOGAI_HOME/packs/actors/prolog"
# The region's recall declares the N8 membership contract, so its construct must be on the path.
